...
namespace DX_WebTemplate
{
    public partial class AccedeAuditViewPage : System.Web.UI.Page
    {
                    ...
                    if (rfp_main_reimburse != null)
                    {
                        ...
                        if (payMethodDesc.PMethod_desc == "Check")
                        {
                            ...
                            if (wfID_p2p != null)
                            {
                                ...
                                if (P2PStatus != null && wfDetails != null && orgRole != null)
                                {
        ...
        public string btnApproveForward(string forwardWF, string remarks, string secureToken)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(encryptedID))
                {
                    ...
                    int ExpID = Convert.ToInt32(wfDetails.Document_Id);
                    ...
                    if (rfp_main != null)
                    {
                        ...
                        var wfDetail_reim = _DataContext.ITP_T_WorkflowActivities
                            .Where(x => x.Document_Id == wfDetails.Document_Id)
                            .Where(x => x.Status == wfDetails.Status)
                            .Where(x => x.AppDocTypeId == rfpDocType)
                            .FirstOrDefault();
        ...
        [WebMethod]
        public static string btnReturnAppClickAjax(string return_remarks, string secureToken)
        {
            AccedeAuditViewPage exp = new AccedeAuditViewPage();
            return exp.btnReturnAppClick(return_remarks, secureToken);
        }

        public string btnReturnAppClick(string return_remarks, string secureToken)
        {
            try
            {
                string encryptedID = secureToken;
                if (!string.IsNullOrEmpty(encryptedID))
                {
                    ...
                    var doc_id = Convert.ToInt32(wfDetails.Document_Id);
                    var exp_details = _DataContext.ACCEDE_T_ExpenseMains
                        .Where(x => x.ID == doc_id)
                        .FirstOrDefault();
                    ...
                    var rfp_main_reimburse = _DataContext.ACCEDE_T_RFPMains
                        .Where(x => x.Exp_ID == exp_details.ID)
                        .Where(x => x.IsExpenseReim == true)
                        .Where(x => x.isTravel != true)
                        .Where(x => x.Status != 4)
                        .FirstOrDefault();
                    ...
                    //            select wfd.WFD_Id;
                    var wfDetails_lastApp = _DataContext.ITP_S_WorkflowDetails.Where(x => x.WF_Id == exp_details.FAPWF_Id)
                        .Where(x => x.Sequence == 1).FirstOrDefault();

                    var wfdID = wfDetails_lastApp.WFD_Id;

                    // CHANGE ORG ROLE ID FROM FAP WF
                    var orgRole = from or in _DataContext.ITP_S_WorkflowDetails
                              where or.WF_Id == exp_details.FAPWF_Id && or.Sequence == 1
                              select or.OrgRole_Id;
                    var orID = (int)orgRole.FirstOrDefault();
