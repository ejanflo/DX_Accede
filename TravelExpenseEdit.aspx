<%@ Page Title="" Language="C#" MasterPageFile="~/Main.master" AutoEventWireup="true" CodeBehind="TravelExpenseEdit.aspx.cs" Inherits="DX_WebTemplate.TravelExpenseEdit" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script>
        function linkToRFP() {
            ...
            $.ajax({
                type: "POST",
                url: "TravelExpenseEdit.aspx/RedirectToRFPDetailsAJAX",
                data: JSON.stringify({
                    rfpDoc: rfpDoc
                }),
        ...
        function AddReimbursement(stat) {
            if (ASPxClientEdit.ValidateGroup('submitValid')) {
                ...
                $.ajax({
                    type: "POST",
                    url: "TravelExpenseEdit.aspx/AddRFPReimburseAJAX",
                    data: JSON.stringify({
                        empname: empname,
                    ...
                    success: function (response) {
                        // Handle success
                        if (response.d == true) {
                            CAGrid.SetEnabled(false);
                            ExpenseGrid.SetEnabled(false);
                            if (stat == 0) {
        ...
        function ReimbursementTrap() {
            ...
            $.ajax({
                type: "POST",
                url: "TravelExpenseEdit.aspx/CheckReimburseValidationAJAX",
                data: JSON.stringify({
                    t_amount: t_amount
                }),
                contentType: "application/json; charset=utf-8",
        ...
        function ExpCancelClick(s, e) {
            ...
            $.ajax({
                type: "POST",
                url: "TravelExpenseEdit.aspx/SaveTotExpAJAX",
                ...
                success: function (response) {
                    ...
                    wfCallback.EndCallback.AddHandler(function () {
                        handleExpCA(response.d.expType, response.d.totalca, response.d.totalexp, response.d.allTot);
        ...
        function handleExpCA(expType, totalca, totalexp, allTot) {
        ...
        function ExpPopupClick(s, e) {
            if (ASPxClientEdit.ValidateGroup("expAdd")) {
                if (!checkUnsavedChanges(reimTranGrid) || !checkUnsavedChanges(fixedAllowGrid) || !checkUnsavedChanges(miscTravelGrid) || !checkUnsavedChanges(otherBusGrid) || !checkUnsavedChanges(entertainmentGrid) || !checkUnsavedChanges(busMealsGrid)) {
                    e.processOnServer = false; // Prevent server-side processing if unsaved changes exist
                } else {
                    LoadingPanel.Show();
                    var locParticulars = locParticularsMemo.GetText();
                    var travelDate = travelDateCalendar.GetSelectedDate()
                    var totalExp = totalExpTB.GetValue()

                    $.ajax({
                        type: "POST",
                        url: "TravelExpenseEdit.aspx/AddTravelExpenseDetailsAJAX",
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        data: JSON.stringify({
        ...
        function checkUnsavedChanges(grid) {
            if (grid.IsEditing()) {
                if (confirm("There are unsaved changes. Do you want to proceed without saving?"))
                    return true;  // Proceed with the button click event
                else
                    return false; // Cancel the button click event
            }
            return true; // No editing, proceed with the button click event
        }
        ...
        function removeFromExp(item_id, btnCommand) {
            $.ajax({
                type: "POST",
                url: "TravelExpenseEdit.aspx/RemoveFromExp_AJAX",
                data: JSON.stringify({
                    item_id: item_id,
                ...
                success: function (response) {
                    ...
                    wfCallback.EndCallback.AddHandler(function () {
                        ...
                        if (btnCommand == 'btnRemoveCA') {
                            ...
                            $('#toast-header').html('Success!');
                            $('#toast-body').html('CA item has been removed successfully.');
                            setTimeout(function () {
                                $('.toast').toast('show');
                            }, 1000);
                            ...
                            $('#toast-header').html('Success!');
                            $('#toast-body').html('Expense item has been removed successfully.');
                            setTimeout(function () {
                                $('.toast').toast('show');
                            }, 1000);
                        } else if (btnCommand == 'btnRemoveReim') {
                            ReimburseGrid.Refresh();

                            $('#toast-header').html('Success!');
                            $('#toast-body').html('Reimbursement item has been removed successfully.');
                            setTimeout(function () {
                                $('.toast').toast('show');
                            }, 1000);
                        }
            ...
            if (capopGrid.GetSelectedRowCount() > 0) {
                ...
                capopGrid.GetSelectedFieldValues("ID", function (selectedValues) {
                    ...
                    if (selectedValues.length > 0) {
                        $.ajax({
                            type: "POST",
                            url: "TravelExpenseEdit.aspx/AddCA_AJAX",
        ...
        function SaveTravelExpenseReport(btn) {
            ...
            $.ajax({
                type: "POST",
                url: "TravelExpenseEdit.aspx/SaveSubmitTravelExpenseAJAX",
                data: JSON.stringify({
                    empname: empname,
                    reportdate: reportdate,
                    company: company,
        ...
        function viewExpDetailModal(expDetailID) {
            $.ajax({
                type: "POST",
                url: "TravelExpenseEdit.aspx/DisplayExpDetailsAJAX",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                data: JSON.stringify({
                        ...
                        <dx:LayoutGroup Caption="" ColSpan="2" GroupBoxDecoration="None" ColCount="2" ColumnCount="2" ColumnSpan="2">
                            <Items>
                                <dx:LayoutGroup Caption="REPORT HEADER DETAILS" ColSpan="1" GroupBoxDecoration="HeadingLine" Width="65%" ColCount="2" ColumnCount="2" RowSpan="2">
                                    <Items>
                                        <dx:LayoutItem Caption="Employee Name" ColSpan="1" FieldName="Employee_Id">
                                            <LayoutItemNestedControlCollection>
                                                <dx:LayoutItemNestedControlContainer runat="server">
                                                    <dx:ASPxComboBox ID="empnameCB" runat="server" DataSourceID="SqlEmpName" Font-Bold="True" Font-Size="Small" TextField="FullName" ValueField="EmpCode" Width="100%" AllowMouseWheel="False" ClientInstanceName="empnameCB">
                                        ...
                                        <dx:LayoutItem Caption="Transaction Type" ColSpan="1">
                                            <LayoutItemNestedControlCollection>
                                                <dx:LayoutItemNestedControlContainer runat="server">
                                                    <dx:ASPxComboBox ID="drpdown_expenseType" runat="server" ClientInstanceName="drpdown_expenseType" DataSourceID="SqlTranType" Font-Bold="True" Font-Size="Small" HorizontalAlign="Left" TextField="Description" ValueField="ExpenseType_ID" Width="100%" ReadOnly="True">
                                                        <DropDownButton Visible="False">
                                                        </DropDownButton>
                                        ...
                                        <dx:LayoutItem Caption="Company" ColSpan="1" FieldName="Company_Id">
                                            <LayoutItemNestedControlCollection>
                                                <dx:LayoutItemNestedControlContainer runat="server">
                                                    <dx:ASPxComboBox ID="companyCB" runat="server" ClientInstanceName="companyCB" DataSourceID="SqlCompany" EnableTheming="True" Font-Bold="True" Font-Size="Small" TextField="CompanyShortName" ValueField="WASSId" Width="100%">
                                                        <DropDownButton Visible="False">
                                                        </DropDownButton>
                                                        <ValidationSettings Display="Dynamic" SetFocusOnError="True" ValidationGroup="ExpenseEdit">
                                                            <RequiredField ErrorText="*Required" IsRequired="True" />
                                        ...
                                        <dx:LayoutItem Caption="Department" ColSpan="1" FieldName="Dep_Code">
                                            <LayoutItemNestedControlCollection>
                                                <dx:LayoutItemNestedControlContainer runat="server">
                                                    <dx:ASPxComboBox ID="departmentCB" runat="server" Font-Bold="True" Font-Size="Small" Width="100%" ClientInstanceName="departmentCB" DataSourceID="SqlDepartment" TextField="DepCode" ValueField="ID">
                                                        <DropDownButton Visible="False">
                                                        </DropDownButton>
                                                        <Border BorderStyle="None" />
                                                        <BorderBottom BorderColor="Black" BorderStyle="Solid" BorderWidth="1px" />
                                ...
                                <dx:LayoutGroup Caption="REIMBURSEMENT DETAILS" ColSpan="1" GroupBoxDecoration="HeadingLine" ClientVisible="False" Name="reimDetails" Width="100%">
                                    <Paddings Padding="0px" PaddingTop="15px" />
                                    <Items>
                                        <dx:LayoutItem Caption="" ColSpan="1" Width="100%">
                                            <LayoutItemNestedControlCollection>
                                                <dx:LayoutItemNestedControlContainer runat="server">
                                                    <dx:ASPxButtonEdit ID="reimTB" runat="server" ClientInstanceName="reimTB" Font-Bold="True" Font-Size="Small" Width="100%">
                                ...
                                <dx:LayoutGroup Caption="CASH ADVANCES" ColSpan="1" Width="50%">
                                    ...
                                    <Items>
                                        <dx:LayoutItem Caption="" ColSpan="1">
                                            <LayoutItemNestedControlCollection>
                                                <dx:LayoutItemNestedControlContainer runat="server">
                                                    <dx:ASPxGridView ID="CAGrid" runat="server" AutoGenerateColumns="False" ClientInstanceName="CAGrid" DataSourceID="sqlExpenseCA" Font-Size="Small" KeyFieldName="ID" OnRowDeleting="CAGrid_RowDeleting" Width="100%">
                                ...
                                <dx:LayoutGroup Caption="EXPENSES" ColSpan="1" GroupBoxDecoration="Box">
                                    ...
                                    <Items>
                                        <dx:LayoutItem Caption="" ColSpan="1">
                                            <LayoutItemNestedControlCollection>
                                                <dx:LayoutItemNestedControlContainer runat="server">
                                                    <dx:ASPxGridView ID="ExpenseGrid" runat="server" AutoGenerateColumns="False" ClientInstanceName="ExpenseGrid" DataSourceID="SqlExpDetails" Font-Size="Small" KeyFieldName="TravelExpenseDetail_ID" Width="100%">
                                                        <ClientSideEvents CustomButtonClick="onCustomButtonClick" ToolbarItemClick="onToolbarItemClick" />
                                                        <SettingsDetail AllowOnlyOneMasterRowExpanded="True" />
                                                        <SettingsContextMenu Enabled="True">
                                                        </SettingsContextMenu>
...
" />
                                                    ...
                                                    <dx:ASPxGridView ID="DocumentGrid" runat="server" AutoGenerateColumns="False" ClientInstanceName="DocumentGrid" DataSourceID="SqlDocs" Font-Size="Small" KeyFieldName="ID" Width="100%">
	...
	WFSequenceGrid.PerformCallback();
	        ...
	        OnWFChanged();
                                                                                            ...
                                                                                            <dx:LayoutItem Caption="" ColSpan="1">
                                                                                                <LayoutItemNestedControlCollection>
                                                                                                    <dx:LayoutItemNestedControlContainer runat="server">
                                                                                                        <dx:ASPxGridView ID="WFSequenceGrid" runat="server" AutoGenerateColumns="False" ClientInstanceName="WFSequenceGrid" DataSourceID="SqlWorkflowSequence" Theme="iOS" Width="100%">
                                                                                                            <SettingsPopup>
                                                                                                                <FilterControl AutoUpdatePosition="False">
                                                                                                                </FilterControl>
                                                                                                            </SettingsPopup>
	...
	WFSequenceGrid.PerformCallback();
        ...
        <%--<div id="expDiv" style="height: 500px; width: 1200px; overflow: scroll;">--%>
        <dx:ASPxPopupControl ID="caPopup" runat="server" FooterText="" HeaderText="Select Cash Advance/s" ClientInstanceName="caPopup" Modal="True" PopupVerticalAlign="WindowCenter" CloseAction="None" CssClass="rounded" Maximized="True" ShowCloseButton="False" MaxWidth="100%">
            <ContentCollection>
	...
	caPopup.Hide();
                            ...
                            <dx:LayoutGroup Caption="" ColCount="2" ColSpan="1" ColumnCount="2" GroupBoxDecoration="Box" Width="100%">
                                <Items>
                                    <dx:LayoutItem Caption="" ColSpan="2" ColumnSpan="2" Width="100%">
                                        <LayoutItemNestedControlCollection>
                                            <dx:LayoutItemNestedControlContainer runat="server">
                                                <dx:ASPxGridView ID="capopGrid" runat="server" AutoGenerateColumns="False" ClientInstanceName="capopGrid" DataSourceID="sqlRFPMainCA" Font-Size="Smaller" KeyFieldName="ID" OnCustomCallback="capopGrid_CustomCallback" Width="100%">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <dx:GridViewDataTextColumn FieldName="Payee" ShowInCustomizationForm="True" VisibleIndex="2" Width="400px">
                                                        </dx:GridViewDataTextColumn>
                                                        <dx:GridViewDataDateColumn Caption="Last Day of Transaction" FieldName="LastDayTransact" ShowInCustomizationForm="True" VisibleIndex="10" Visible="False">
                                                            <PropertiesDateEdit DisplayFormatString="MMMM dd, yyyy">
                                                            </PropertiesDateEdit>
                                                        </dx:GridViewDataDateColumn>
                                                        ...
                                                        <dx:GridViewDataComboBoxColumn Caption="Payment Method" FieldName="PayMethod" ShowInCustomizationForm="True" VisibleIndex="6">
                                                            <PropertiesComboBox DataSourceID="sqlPayMethod" TextField="PMethod_name" ValueField="ID">
                                                            </PropertiesComboBox>
                                                        </dx:GridViewDataComboBoxColumn>
                                                    </Columns>
	...
	AddReimbursement();
        ...
        <dx:ASPxPopupControl ID="reimbursePopup2" runat="server" AllowDragging="True" AutoUpdatePosition="True" ClientInstanceName="reimbursePopup2" CloseAction="CloseButton" CloseOnEscape="True" HeaderText="Create RFP" Modal="True" PopupAnimationType="None" PopupHorizontalAlign="WindowCenter" PopupVerticalAlign="WindowCenter">
...
" />
                                ...
                                <dx:ASPxFormLayout ID="ASPxFormLayout6" runat="server" Height="450px" Width="100%">
                                    <Items>
                                        ...
                                        <dx:LayoutGroup Caption="" ColCount="2" ColSpan="1" ColumnCount="2" GroupBoxDecoration="None" HorizontalAlign="Center" Width="100%">
                                            ...
                                            <Items>
                                                <dx:LayoutItem Caption="Location/Particulars" ColSpan="1" VerticalAlign="Top">
                                                    <LayoutItemNestedControlCollection>
                                                        <dx:LayoutItemNestedControlContainer runat="server">
                                                            <dx:ASPxMemo ID="locParticularsMemo" runat="server" ClientInstanceName="locParticularsMemo" Theme="MaterialCompact" Width="100%">
                                                ...
                                                <dx:LayoutGroup Caption="" ColCount="2" ColSpan="1" ColumnCount="2" GroupBoxDecoration="None" RowSpan="2" VerticalAlign="Top">
                                                    <Items>
                                                                            ...
                                                                            <dx:GridViewBandColumn Caption="FIXED ALLOWANCE/S" ShowInCustomizationForm="True" VisibleIndex="1">
                                                                                ...
                                                                                <Columns>
                                                                                    ...
                                                                                    <dx:GridViewDataComboBoxColumn Caption="F or P" FieldName="FixedAllow_ForP" ShowInCustomizationForm="True" VisibleIndex="3" Width="230px">
                                                                                        <PropertiesComboBox>
                                                                                            <Items>
                                                                                                <dx:ListEditItem Text="Fixed RET Allowance" Value="F" />
                                                                                                <dx:ListEditItem Text="Per Diem Allowance" Value="P" />
                                                                                            </Items>
                                                                                            <ValidationSettings Display="Dynamic" ErrorTextPosition="Bottom" SetFocusOnError="True" ValidationGroup="expAdd">
                                                                                                <RequiredField ErrorText="*Required" IsRequired="True" />
                                                                                            </ValidationSettings>
	...
	OtherBusinessExpSpecify.SetVisible(false);
                                                ...
                                                <dx:LayoutItem Caption="Date" ColSpan="1" VerticalAlign="Top">
                                                    <LayoutItemNestedControlCollection>
                                                        <dx:LayoutItemNestedControlContainer runat="server">
                                                            <dx:ASPxCalendar ID="travelDateCalendar" runat="server" ClientInstanceName="travelDateCalendar" EnableLargePeriodNavigation="False" HighlightToday="False" Theme="MaterialCompact" Width="100%">
                                                                <DaySelectedStyle BackColor="#006838" ForeColor="White">
                                                                </DaySelectedStyle>
                                                                <ButtonStyle>
                                                                    <HoverStyle ForeColor="#006838">
                                                                    </HoverStyle>
                                                                </ButtonStyle>
    ...
    <asp:SqlDataSource ID="sqlExpenseCA" runat="server" ConnectionString="<%$ ConnectionStrings:ITPORTALConnectionString %>" SelectCommand="SELECT * FROM [ACCEDE_T_RFPMain] WHERE (([IsExpenseCA] = @IsExpenseCA) AND ([Exp_ID] = @Exp_ID) AND ([Status] = @Status) AND ([isTravel] = 1)) ">
        <SelectParameters>
            ...
            <asp:Parameter DefaultValue="7" Name="Status" Type="Int32" />
        </SelectParameters>
    </asp:SqlDataSource>
    ...
    <asp:SqlDataSource ID="SqlRTMap" runat="server" ConnectionString="<%$ ConnectionStrings:ITPORTALConnectionString %>" DeleteCommand="DELETE FROM [ACCEDE_T_TraExpReimTranspoMap] WHERE [ReimTranspo_ID] = @ReimTranspo_ID" InsertCommand="INSERT INTO [ACCEDE_T_TraExpReimTranspoMap] ([ReimTranspo_Type], [ReimTranspo_Amount], [TravelExpenseDetail_ID], [User_ID]) VALUES (@ReimTranspo_Type, @ReimTranspo_Amount, @TravelExpenseDetail_ID, @User_ID)" SelectCommand="SELECT * FROM [ACCEDE_T_TraExpReimTranspoMap] WHERE (([TravelExpenseDetail_ID] = @TravelExpenseDetail_ID) AND ([User_ID] = @User_ID)) ORDER BY [ReimTranspo_ID]" UpdateCommand="UPDATE [ACCEDE_T_TraExpReimTranspoMap] SET [ReimTranspo_Type] = @ReimTranspo_Type, [ReimTranspo_Amount] = @ReimTranspo_Amount, [TravelExpenseDetail_ID] = @TravelExpenseDetail_ID, [User_ID] = @User_ID WHERE [ReimTranspo_ID] = @ReimTranspo_ID">
        <DeleteParameters>
    ...
    <asp:SqlDataSource ID="SqlFAMap" runat="server" ConnectionString="<%$ ConnectionStrings:ITPORTALConnectionString %>" DeleteCommand="DELETE FROM [ACCEDE_T_TraExpFixedAllowMap] WHERE [FixedAllow_ID] = @FixedAllow_ID" InsertCommand="INSERT INTO [ACCEDE_T_TraExpFixedAllowMap] ([FixedAllow_ForP], [FixedAllow_Amount], [TravelExpenseDetail_ID], [User_ID]) VALUES (@FixedAllow_ForP, @FixedAllow_Amount, @TravelExpenseDetail_ID, @User_ID)" SelectCommand="SELECT * FROM [ACCEDE_T_TraExpFixedAllowMap] WHERE (([TravelExpenseDetail_ID] = @TravelExpenseDetail_ID) AND ([User_ID] = @User_ID)) ORDER BY [FixedAllow_ID]" UpdateCommand="UPDATE [ACCEDE_T_TraExpFixedAllowMap] SET [FixedAllow_ForP] = @FixedAllow_ForP, [FixedAllow_Amount] = @FixedAllow_Amount, [TravelExpenseDetail_ID] = @TravelExpenseDetail_ID, [User_ID] = @User_ID WHERE [FixedAllow_ID] = @FixedAllow_ID">
        <DeleteParameters>
    ...
    <asp:SqlDataSource ID="SqlEMap" runat="server" ConnectionString="<%$ ConnectionStrings:ITPORTALConnectionString %>" DeleteCommand="DELETE FROM [ACCEDE_T_TraExpEntertainmentMap] WHERE [Entertainment_ID] = @Entertainment_ID" InsertCommand="INSERT INTO [ACCEDE_T_TraExpEntertainmentMap] ([Entertainment_Explain], [Entertainment_Amount], [TravelExpenseDetail_ID], [User_ID]) VALUES (@Entertainment_Explain, @Entertainment_Amount, @TravelExpenseDetail_ID, @User_ID)" SelectCommand="SELECT * FROM [ACCEDE_T_TraExpEntertainmentMap] WHERE (([TravelExpenseDetail_ID] = @TravelExpenseDetail_ID) AND ([User_ID] = @User_ID)) ORDER BY [TravelExpenseDetail_ID]" UpdateCommand="UPDATE [ACCEDE_T_TraExpEntertainmentMap] SET [Entertainment_Explain] = @Entertainment_Explain, [Entertainment_Amount] = @Entertainment_Amount, [TravelExpenseDetail_ID] = @TravelExpenseDetail_ID, [User_ID] = @User_ID WHERE [Entertainment_ID] = @Entertainment_ID">
        <DeleteParameters>
