...
namespace DX_WebTemplate
{
    public partial class TravelExpenseAdd : System.Web.UI.Page
    {
        ...
        public static object AddCA_AJAX(List<int> selectedValues)
        {
            try
            {
                var handler = new TravelExpenseAdd();
                return handler.AddCA(selectedValues);
            }
            catch
            {
                return string.Empty;
        ...
        public object AddCA(List<int> selectedIds)
        {
            try
            {
                ...
                var expMain = _DataContext.ACCEDE_T_TravelExpenseMains
                    .FirstOrDefault(x => x.ID == travelExpId);
                if (expMain != null)
                {
                    expMain.ExpenseType_ID = 1;
                }

                var rfps = _DataContext.ACCEDE_T_RFPMains
                    .Where(rfp => selectedIds.Contains(rfp.ID) && rfp.User_ID == userId.ToString())
                    .ToList();

                foreach (var id in selectedIds)
                {
                    var ca = new ACCEDE_T_ExpenseCA
                    {
                        RFPMain_ID = id,
                        User_ID = userId,
                        IsUploaded = false
        ...
        protected void ASPxGridView22_RowUpdating(object sender, ASPxDataUpdatingEventArgs e)
        {
            ...
            if (Session["expAction"].ToString() == "edit")
            {
              ...
              SET ReimTranspo_Type1 = {0}, 
                  ...
                  LocParticulars = {19}
              WHERE TravelExpenseDetailMap_ID = {20}",
                    ...
                    Convert.ToDecimal(e.NewValues["LocParticulars"]),
        ...
        [WebMethod]
        public static object AddTravelExpenseDetailsAJAX(DateTime travelDate, string totalExp)
        {
            TravelExpenseAdd exp = new TravelExpenseAdd();
            return exp.AddTravelExpenseDetails(travelDate, totalExp);
        }

        public object AddTravelExpenseDetails(DateTime travelDate, string totalExp)
        {
            try
            {
